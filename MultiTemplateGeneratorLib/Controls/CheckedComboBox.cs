...
namespace MultiTemplateGeneratorLib.Controls
{
    public class CheckedComboBox : ComboBox
    {
        ...
        public void SetItemsChecked(string valueSeparatedText, char separator)
        {
            var parts = valueSeparatedText.Split(new[] { separator }, StringSplitOptions.RemoveEmptyEntries);
            foreach (var part in parts) SetItemChecked(part.Trim(), true);
        }

        public bool SetItemChecked(string text, bool isChecked)
        {
            var indexItem = Items.Cast<CCBoxItem>().SingleOrDefault(x => x.Name == text);
            if (indexItem == null)
                return false;
            var index = Items.IndexOf(indexItem);
            if (index < 0 || index > Items.Count) throw new ArgumentOutOfRangeException(nameof(text), "value out of range");
